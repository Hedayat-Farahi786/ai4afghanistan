...
const menuItems = [
  ...
  { title: 'Pages', links: [
      ...
      { path: '/pages/contacts', label: 'Contact' },
    ...
    <ul className="mobile-nav-list nav-list1">
      {menuItems.map((item, index) => (
        <li key={index} className={`has-sub ${openIndex === index ? 'submenu-opened' : ''}`}>
          {item.links ? (
            <>
              <div className="d-flex align-items-center justify-content-between" onClick={() => toggleMenu(index)}>
                <a href="#">{item.title}</a>
                <span>{openIndex === index ? <FaMinus /> : <FaPlus />}</span>
              </div>
              <ul className={`sub-menu ${openIndex === index ? 'open-sub sub-menu-active d-block' : ''}`}>
                {item.links.map((link, linkIndex) => (
                  <li key={linkIndex}><Link href={link.path}>{link.label}</Link></li>
                ))}
              </ul>
            </>
          ) : (
